...
class SpeakerManager:
    ...
    def compute_d_vector_from_clip(self, wav_file: Union[str, list]) -> list:
        ...
        d_vector = None
        idx = 0
        num_files = len(wav_file)

        if num_files == 1:
            d_vector = self.compute_speaker_embedding(wav_file[0])
            return d_vector.tolist()

        embeddings = self.compute_speaker_embedding(wav_file[0])
        for elem in wav_file:
            embedding = self.compute_speaker_embedding(elem)
